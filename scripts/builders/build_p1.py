import os
from scripts.builders.file_writer import write_file

def build():
    # vector2d
    v2d = """from __future__ import annotations
import math
...
class Vector2D:
    ...
    def __truediv__(self, scalar: Union[int, float, Vector2D]) -> Vector2D:
        if isinstance(scalar, (int, float)):
            if abs(scalar) < EPSILON: raise ZeroDivisionError("Cannot divide Vector2D by zero scalar.")
            return Vector2D(self.x / scalar, self.y / scalar)
        if isinstance(scalar, Vector2D):
            if abs(scalar.x) < EPSILON or abs(scalar.y) < EPSILON: raise ZeroDivisionError("Cannot divide Vector2D by zero component.")
...
"""
    write_file("client/math/vector2d.py", v2d)
    print("Vector2D generated.")
