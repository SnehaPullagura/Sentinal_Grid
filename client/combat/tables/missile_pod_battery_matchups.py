"""
Combat Matchup & Tactical Evaluation Engine: MISSILE_POD_BATTERY
...
class MissilePodBatteryMatchupMatrix:
    def __init__(self):
        self.tower_id: str = "missile_pod_battery"
        self.base_damage: float = 110.0
        self.fire_rate: float = 2.5
        self.range_radius: float = 160.0
    ...
    def _build_matchup_database(self) -> Dict[str, MatchupEvaluation]:
        enemies = [
            ...
            ("aero_interceptor", 95.0, 0.0, 0.0, "DISFAVORED" if "missile_pod_battery" != "flak_anti_air" else "FAVORED", "High altitude tracking required"),
        ...
        for eid, hp, sh, arm, rating, pos_hint in enemies:
            ...
            db[eid] = MatchupEvaluation(
                ...
                elemental_synergy="Thermal Shock + Cryo" if "missile_pod_battery".startswith("plasma") else "Standard Kinetic Burst"
            )
        return db
