"""
Combat Matchup & Tactical Evaluation Engine: HEAVY_DEFENSE_MATRIX
...
class HeavyDefenseMatrixMatchupMatrix:
    def __init__(self):
        self.tower_id: str = "heavy_defense_matrix"
        self.base_damage: float = 115.0
        self.fire_rate: float = 3.0
        self.range_radius: float = 180.0
    ...
    def _build_matchup_database(self) -> Dict[str, MatchupEvaluation]:
        enemies = [
            ...
            ("aero_interceptor", 95.0, 0.0, 0.0, "DISFAVORED" if "heavy_defense_matrix" != "flak_anti_air" else "FAVORED", "High altitude tracking required"),
        ...
        for eid, hp, sh, arm, rating, pos_hint in enemies:
            ...
            db[eid] = MatchupEvaluation(
                ...
                elemental_synergy="Thermal Shock + Cryo" if "heavy_defense_matrix".startswith("plasma") else "Standard Kinetic Burst"
            )
        return db
