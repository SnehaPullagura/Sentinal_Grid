"""
Campaign Mission Model 08: Tactical Scenario Data & Execution Pipeline.
"""
...
class Mission08DataModel:
    def __init__(self):
        self.mission_index: int = 8
        self.sector_name: str = "Tactical Sector 08 - Iron Outpost"
        self.world_tier: int = 2
        self.base_starting_credits: int = 560
        self.base_starting_energy: int = 140
        self.waves_data: Dict[int, SectorWaveConfig] = self._compile_waves()

    def _compile_waves(self) -> Dict[int, SectorWaveConfig]:
        waves = {}
        for w in range(1, 19):
            threat = round(20.0 + w * 12.5 + 8 * 3.5, 1)
            spawns = [
