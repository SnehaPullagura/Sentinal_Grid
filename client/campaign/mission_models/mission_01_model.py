"""
Campaign Mission Model 01: Tactical Scenario Data & Execution Pipeline.
"""
...
class Mission01DataModel:
    def __init__(self):
        self.mission_index: int = 1
        self.sector_name: str = "Tactical Sector 01 - Iron Outpost"
        self.world_tier: int = 1
        self.base_starting_credits: int = 420
        self.base_starting_energy: int = 105
        self.waves_data: Dict[int, SectorWaveConfig] = self._compile_waves()

    def _compile_waves(self) -> Dict[int, SectorWaveConfig]:
        waves = {}
        for w in range(1, 12):
            threat = round(20.0 + w * 12.5 + 1 * 3.5, 1)
            spawns = [
