"""
Campaign Mission Model 02: Tactical Scenario Data & Execution Pipeline.
"""
...
class Mission02DataModel:
    def __init__(self):
        self.mission_index: int = 2
        self.sector_name: str = "Tactical Sector 02 - Iron Outpost"
        self.world_tier: int = 1
        self.base_starting_credits: int = 440
        self.base_starting_energy: int = 110
        self.waves_data: Dict[int, SectorWaveConfig] = self._compile_waves()

    def _compile_waves(self) -> Dict[int, SectorWaveConfig]:
        waves = {}
        for w in range(1, 13):
            threat = round(20.0 + w * 12.5 + 2 * 3.5, 1)
            spawns = [
