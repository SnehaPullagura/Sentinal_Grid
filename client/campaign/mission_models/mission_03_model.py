"""
Campaign Mission Model 03: Tactical Scenario Data & Execution Pipeline.
"""
...
class Mission03DataModel:
    def __init__(self):
        self.mission_index: int = 3
        self.sector_name: str = "Tactical Sector 03 - Iron Outpost"
        self.world_tier: int = 1
        self.base_starting_credits: int = 460
        self.base_starting_energy: int = 115
        self.waves_data: Dict[int, SectorWaveConfig] = self._compile_waves()

    def _compile_waves(self) -> Dict[int, SectorWaveConfig]:
        waves = {}
        for w in range(1, 14):
            threat = round(20.0 + w * 12.5 + 3 * 3.5, 1)
            spawns = [
