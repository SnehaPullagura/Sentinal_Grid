"""
Campaign Mission Model 06: Tactical Scenario Data & Execution Pipeline.
"""
...
class Mission06DataModel:
    def __init__(self):
        self.mission_index: int = 6
        self.sector_name: str = "Tactical Sector 06 - Iron Outpost"
        self.world_tier: int = 2
        self.base_starting_credits: int = 520
        self.base_starting_energy: int = 130
        self.waves_data: Dict[int, SectorWaveConfig] = self._compile_waves()

    def _compile_waves(self) -> Dict[int, SectorWaveConfig]:
        waves = {}
        for w in range(1, 17):
            threat = round(20.0 + w * 12.5 + 6 * 3.5, 1)
            spawns = [
