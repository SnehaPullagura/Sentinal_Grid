"""
Campaign Mission Model 07: Tactical Scenario Data & Execution Pipeline.
"""
...
class Mission07DataModel:
    def __init__(self):
        self.mission_index: int = 7
        self.sector_name: str = "Tactical Sector 07 - Iron Outpost"
        self.world_tier: int = 2
        self.base_starting_credits: int = 540
        self.base_starting_energy: int = 135
        self.waves_data: Dict[int, SectorWaveConfig] = self._compile_waves()

    def _compile_waves(self) -> Dict[int, SectorWaveConfig]:
        waves = {}
        for w in range(1, 18):
            threat = round(20.0 + w * 12.5 + 7 * 3.5, 1)
            spawns = [
