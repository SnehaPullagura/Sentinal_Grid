"""
Campaign Mission Model 09: Tactical Scenario Data & Execution Pipeline.
"""
...
class Mission09DataModel:
    def __init__(self):
        self.mission_index: int = 9
        self.sector_name: str = "Tactical Sector 09 - Iron Outpost"
        self.world_tier: int = 2
        self.base_starting_credits: int = 580
        self.base_starting_energy: int = 145
        self.waves_data: Dict[int, SectorWaveConfig] = self._compile_waves()

    def _compile_waves(self) -> Dict[int, SectorWaveConfig]:
        waves = {}
        for w in range(1, 20):
            threat = round(20.0 + w * 12.5 + 9 * 3.5, 1)
            spawns = [
