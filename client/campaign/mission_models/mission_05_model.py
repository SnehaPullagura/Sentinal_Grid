"""
Campaign Mission Model 05: Tactical Scenario Data & Execution Pipeline.
"""
...
class Mission05DataModel:
    def __init__(self):
        self.mission_index: int = 5
        self.sector_name: str = "Tactical Sector 05 - Iron Outpost"
        self.world_tier: int = 1
        self.base_starting_credits: int = 500
        self.base_starting_energy: int = 125
        self.waves_data: Dict[int, SectorWaveConfig] = self._compile_waves()

    def _compile_waves(self) -> Dict[int, SectorWaveConfig]:
        waves = {}
        for w in range(1, 16):
            threat = round(20.0 + w * 12.5 + 5 * 3.5, 1)
            spawns = [
