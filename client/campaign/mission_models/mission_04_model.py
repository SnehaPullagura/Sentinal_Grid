"""
Campaign Mission Model 04: Tactical Scenario Data & Execution Pipeline.
"""
...
class Mission04DataModel:
    def __init__(self):
        self.mission_index: int = 4
        self.sector_name: str = "Tactical Sector 04 - Iron Outpost"
        self.world_tier: int = 1
        self.base_starting_credits: int = 480
        self.base_starting_energy: int = 120
        self.waves_data: Dict[int, SectorWaveConfig] = self._compile_waves()

    def _compile_waves(self) -> Dict[int, SectorWaveConfig]:
        waves = {}
        for w in range(1, 15):
            threat = round(20.0 + w * 12.5 + 4 * 3.5, 1)
            spawns = [
